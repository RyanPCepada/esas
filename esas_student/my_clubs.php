    ...
    <script>
        ...
        $(document).ready(function() {
                            ...
                            clubsContainer.innerHTML = response.map(club => `
                                <div class="col-md-4 card-container"> 
                                    ...
                                    <div class="card card-img-only">
                                        <a href="${tab === 'active' ? `/esas/esas_student/home.php?club_id=${club.club_id}&application_id=${club.application_id}` : `/esas/esas_student/application_details.php?club_id=${club.club_id}&club_name=${encodeURIComponent(club.clubName)}&status=${tab}&application_id=${club.application_id}`}" 
            ...
            function markClubNotificationsAsRead(club_id) {
                $.ajax({
                    url: '/esas/esas_student/apis/notifications/club-notifications-mark-read.php',
                    method: 'POST',
                    data: { club_id: club_id },
                    success: function(response) {
                        const data = JSON.parse(response);
                        if (data.status === 'success') {
                            $(`#club-notification-${club_id}`).hide(); // Hide the badge after marking as read
                        } else {
