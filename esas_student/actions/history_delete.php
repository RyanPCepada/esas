...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['clear_all'])) {
        // Prepare the SQL statement to delete all activities for the current student
        $sql = "DELETE FROM tbl_activity_logs WHERE student_id = ?";
        ...
        if (isset($_POST['activity_ids']) && is_array($_POST['activity_ids'])) {
            ...
            $sql = "DELETE FROM tbl_activity_logs WHERE activity_id IN ($placeholders) AND student_id = ?";
            $stmt = $pdo->prepare($sql);
            
            // Execute the statement with the activity IDs and the student ID
            $stmt->execute(array_merge($activity_ids, [$student_id]));
    ...
    // Prepare the SQL statement for deletion
    $sql = "DELETE FROM tbl_activity_logs WHERE activity_id = ? AND student_id = ?";
    $stmt = $pdo->prepare($sql);
...
header("Location: /esas/esas_student/history.php");
exit;
//HERE
?>
