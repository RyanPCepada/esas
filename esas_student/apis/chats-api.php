...
try {
    ...
    // Query to fetch active students and moderators in the specified club
    $stmt = $pdo->prepare("
        SELECT 
            s.student_id, 
            s.firstName, 
        ...
        LEFT JOIN 
            ...
            AND c.recipient_id = :student_id  -- Ensure chats where the student is the recipient
        WHERE 
            r.club_id = :club_id 
            AND r.status = 'active'
        GROUP BY 
        ...
        LEFT JOIN 
            ...
            AND c.club_id = :club_id
            AND c.recipient_id = :student_id -- Ensure chats where the current student is the recipient
        WHERE 
            cm.club_id = :club_id 
            AND m.moderator_id != :student_id -- Exclude the current student from the moderator list
        GROUP BY 
            m.moderator_id 
        ORDER BY 
