...
<head>
    ...
    <title>eSAS - Departure Requests</title>
...
<body>
            ...
            <div class="col-12 col-md-2 pt-3 border-end">
                ...
                <div class="d-flex flex-column flex-shrink-0 px-2 bg-body-tertiary">
                    <ul class="nav nav-pills flex-column mb-auto">
                        ...
                        <li class="nav-item">
                            <a href="../../esas_moderator/public/students.php" class="nav-link left-sidebar text-dark" id="my-clubs">
    ...
    $selectedMonth = isset($_GET['month']) ? intval($_GET['month']) : date('m'); // Month from URL or current month

    // SQL query to fetch distinct students with their pending departure requests
    $sql = "SELECT 
                s.student_id,
                s.firstName,
                s.middleName,
                ...
                s.course,
                s.profilePic,
                MIN(dr.dateRequested) AS dateRequested,
                c.club_id,
                dr.departure_id
            FROM tbl_students s
            LEFT JOIN tbl_departure_requests dr ON s.student_id = dr.student_id
            LEFT JOIN tbl_clubs c ON dr.club_id = c.club_id
    ...
    if ($selectedMonth) {
        $sql .= " AND MONTH(dr.dateRequested) <= :selectedMonth";
    }

    $sql .= " GROUP BY s.student_id
            ORDER BY MIN(dr.dateRequested) ASC"; // Order by the earliest date requested
    ...
    if ($totalRows > 0) {
        ...
        while ($row = $stmt->fetch()) {
            ...
            <tr class="student-row">
                ...
                <td class="text-center">
                    <a href="../public/crud/departure_requests/departure_request_read.php?student_id=' . htmlspecialchars($row['student_id']) . '&club_id=' . htmlspecialchars($row['club_id']) . '&departure_id=' . $departureId . '" class="mr-2" title="View Record" data-toggle="tooltip"><span class="fa fa-eye"></span></a>
                </td>
            </tr>';
        }
        ...
        echo '<p style="font-size: 16px; color: red;"><em>No departure requests found.</em></p>';
    }
?>
