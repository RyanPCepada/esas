    ...
    $(document).ready(function() {
        ...
        function buildCards(data, type) {
            ...
            return members.map(member => {
                ...
                if (type === 'moderator') {
                    // Moderator details
                    const [fullName, profilePic, department, dateAssigned] = details; // Make sure dateAssigned is being set from the SQL
                    const nameParts = fullName.split(' ');  // Split the fullName by spaces
                    const firstName = nameParts[0];  // The first element is the first name
                    const lastName = nameParts.length > 1 ? nameParts[nameParts.length - 1] : '';  // The last element is the last name
                    const middleName = nameParts.length > 2 ? nameParts.slice(1, -1).join(' ') : '';  // All elements in between are the middle name
                                    ...
                                    <div class="col-10">
                                        <div class="card-body p-0">
                                            <div class="d-flex flex-column">
                                                ...
                                                <p class="mb-0">Assigned: ${new Date(dateAssigned).toLocaleDateString('en-US', {
                                                    year: 'numeric', month: 'long', day: 'numeric'
                                                })}</p>
                                            </div>
                                        </div>
